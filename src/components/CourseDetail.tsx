...
import { ArrowLeft, Calendar, Clock, CheckCircle, Circle, Loader2, AlertCircle } from 'lucide-react';
...
export const CourseDetail: React.FC<CourseDetailProps> = ({ course, onBack }) => {
  ...
  const [selectedTopic, setSelectedTopic] = useState<{ day: number; hour: number; topic: string } | null>(null);
  ...
  const totalTopics = courseContent.schedule.reduce((sum, day) => sum + day.topics.length, 0);
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">{courseContent.course_name}</h1>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {day.topics.map((topic) => {
                  ...
                  return (
                    <div
                      key={topic.hour}
                      onClick={() => handleTopicClick(day.day, topic.hour, topic.topic)}
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        <span className="text-sm font-medium text-blue-600">Hour {topic.hour}</span>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                      ...
                      <h3 className={`text-sm font-medium group-hover:text-blue-600 transition-colors duration-200 ${
